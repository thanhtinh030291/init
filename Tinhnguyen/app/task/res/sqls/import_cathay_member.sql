...
(
    SELECT DISTINCT
        ...
        memb.eff_date paid_from_date,
        memb.exp_date paid_to_date,
        NULL reverse_date
    FROM members memb
),
debit_notes AS
(
    SELECT
        ...
        note.paid_from_date,
        note.paid_to_date
    FROM bf_paid_note note
        JOIN members memb
          ON memb.mepl_oid = note.mepl_oid
         AND memb.op_ind = 'Yes'
        JOIN mr_member_plan mepl
          ON note.mepl_oid = mepl.mepl_oid
...
SELECT DISTINCT
    ...
    TO_CHAR(MAX(pano.paid_to_date), 'YYYY-MM-DD') "memb_exp_date",
        ...
        CASE
            WHEN EXISTS(
                ...
                WHERE mepl_oid = memb.mepl_oid
                  AND (
                    CURRENT_DATE BETWEEN paid_from_date AND paid_to_date OR
                    memb.exp_date <= paid_to_date
                  )
            ) THEN 'Yes'
            ELSE 'No'
...
FROM members memb
    ...
    INNER JOIN (
     ...
     LEFT JOIN bf_paid_note pano
            ON pano.mepl_oid = memb.mepl_oid
    INNER JOIN (
        SELECT
            pocy_no,
