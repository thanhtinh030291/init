 ...
 */
class ClientApiMemberView extends ClientApiV10View
{
    ...
    public function postRegister($request, $response)
    {
        ...
        if ( ! isset($data['language']) OR $data['language'] != 'vi')
        {
            $data['language'] = '';
        }
        else
        ...
        # Validation Policy No and Email
        if (empty($data['pocy_no']))
        {
            return $response->withStatus(400)->withJson([
                'code' => 1,
                'message' => $this->i18n->pocyNoRequired
            ]);
        }
        ...
        if ($rp_check_pocy == false)
        {
            return $response->withStatus(200)->withJson([
                'code' => 101,
                'message' => $this->i18n->pocyNotExist
            ]);
        }
        
        # get member by using email and Policy No
        $members = $this->getMembers($data['pocy_no'], $data['email']);
        if (count($members) != 1)
        {
            return $response->withStatus(200)->withJson([
                'code' => 10,
        ...
        # Member is unique, let create account to this member
        $member = $members[0];
        $password = uniqid();
        $result = $this->doCreateMobileUser($data['email'], $password, $member['company'], $member['mbr_no'], $data['language'], [], null);
        ...
        return $response->withStatus(200)->withJson([
            'code' => 9,
            'message' => $this->i18n->registerSuccess($data['email'])
        ]);
    }
    ...
    public function postEkyc($request, $response, $args)
    {
        ...
        if($rp_api['code'] != 0){
            ...
            $dob = $this->doParserDate($rp_api['dob']);
            if($dob == false){
                return $response->withStatus(400)->withJson([
                    'code' => 400,
                    'message' => $this->i18n->id_font_requid
    ...
    /**
     * @api GET /api/member/info/{mbr_no}/{company}
     *
     * @throws
     */
    public function getInfo($request, $response, $args)
    {
        ...
        $memberNo = $args['param_0'];
        $company = $args['param_1'];
        $info = $this->doGetInfo($memberNo, $company);
        if ($info === false)
        {
            return $response->withStatus(500)->withJson([
                'code' => 500,
    ...
    public function getBenefits($request, $response, $args)
    {
        ...
        $mepl_oid = $args['param_0'];
        $company = $args['param_1'];
        $lang = $request->getHeaderLine(self::HEADER_LANGUAGE);
        $benefits = $this->doGetBenefits($mepl_oid, $company, $lang);
