...
class ClientApiMemberGetInfoPresenter extends ClientApiMemberPresenter
{
     ...
     * @throws
     */
    public function doGetInfo($memberNo)
    {
        ...
        $member['extra'] = $this->getInfo($memberNo);
        return $member;
    }
     ...
     * @throws
     */
    protected function getInfo($memberNo)
    {
        $model = ModelPool::getModel('HbsMember');
        $rows = $model->where([
			'company' => 'pcv',
            'trim(leading 0 from mbr_no)' => intval(str_replace('-', '', $memberNo))
        ]);
