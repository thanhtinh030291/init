SELECT * FROM (
    SELECT
        ...
        CAST(MAX(IF(name = 'providerLatitude', value, NULL)) as DOUBLE) AS `providerLatitude`,
        CAST(MAX(IF(name = 'providerLongitude', value, NULL)) as DOUBLE) AS `providerLongitude`,
        ...
        CAST(MAX(IF(name = 'providerAmount', value, NULL)) as DOUBLE) AS `providerAmount`,
        ...
        CAST(MAX(IF(name = 'providersPriceFrom', value, NULL)) as DOUBLE) AS `providersPriceFrom`,
        CAST(MAX(IF(name = 'providersPriceTo', value, NULL)) as DOUBLE) AS `providersPriceTo`,
        MAX(IF(NAME = 'providerNickname1', VALUE, NULL)) AS `providerNickname1`,
        lang
    FROM (
        SELECT
            item.id,
