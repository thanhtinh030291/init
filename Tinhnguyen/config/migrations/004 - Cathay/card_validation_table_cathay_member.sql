...
ALTER TABLE `cathay_member` ADD FULLTEXT(`memb_rstr`, `memb_rstr_vi`);
...
DELIMITER $$
CREATE TRIGGER `cathay_member__id` BEFORE INSERT ON `cathay_member` FOR EACH ROW BEGIN IF NEW.id IS NULL OR NEW.id = '' THEN SET NEW.id = UUID(); END IF; SET @last_uuid = NEW.id; END
$$
DELIMITER ;
