...
DELIMITER $$
CREATE TRIGGER `cathay_claim_line__id` BEFORE INSERT ON `cathay_claim_line` FOR EACH ROW BEGIN IF NEW.id IS NULL OR NEW.id = '' THEN SET NEW.id = UUID(); END IF; SET @last_uuid = NEW.id; END
$$
DELIMITER ;
