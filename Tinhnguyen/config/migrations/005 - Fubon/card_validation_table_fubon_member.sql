...
ALTER TABLE `fubon_member` ADD FULLTEXT(`memb_rstr`, `memb_rstr_vi`);
...
DELIMITER $$
CREATE TRIGGER `fubon_member__id` BEFORE INSERT ON `fubon_member` FOR EACH ROW BEGIN IF NEW.id IS NULL OR NEW.id = '' THEN SET NEW.id = UUID(); END IF; SET @last_uuid = NEW.id; END
$$
DELIMITER ;
