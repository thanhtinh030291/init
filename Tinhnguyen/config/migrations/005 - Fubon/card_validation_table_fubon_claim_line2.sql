...
DELIMITER $$
CREATE TRIGGER `fubon_claim_line2__id` BEFORE INSERT ON `fubon_claim_line2` FOR EACH ROW BEGIN IF NEW.id IS NULL OR NEW.id = '' THEN SET NEW.id = UUID(); END IF; SET @last_uuid = NEW.id; END
$$
DELIMITER ;
