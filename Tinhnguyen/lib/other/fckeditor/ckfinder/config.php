...
$dir = str_replace("lib{$ds}other{$ds}fckeditor{$ds}ckfinder", '', __DIR__);

require_once "$dir{$ds}lib{$ds}vendor{$ds}autoload.php";
require_once "$dir{$ds}config{$ds}environment{$ds}config.php";
require_once "$dir{$ds}core{$ds}utility{$ds}pattern{$ds}Singleton.class.php";
require_once "$dir{$ds}core{$ds}utility{$ds}data{$ds}DatabasePool.class.php";
require_once "$dir{$ds}core{$ds}utility{$ds}data{$ds}Setting.class.php";
require_once "$dir{$ds}core{$ds}utility{$ds}data{$ds}SessionHandler.class.php";
require_once "$dir{$ds}core{$ds}utility{$ds}pattern{$ds}DIContainer.class.php";
require_once "$dir{$ds}core{$ds}utility{$ds}text{$ds}Sql.class.php";
require_once "$dir{$ds}config{$ds}models{$ds}BaseModel.class.php";
require_once "$dir{$ds}config{$ds}models{$ds}LzaModel.class.php";
require_once "$dir{$ds}config{$ds}models{$ds}LzasettingModel.class.php";
require_once "$dir{$ds}config{$ds}models{$ds}ModelPool.class.php";

$session = DIContainer::bindObject('session', SessionHandler::class);
$setting = DIContainer::bindObject('setting', Setting::class);
...
function CheckAuthentication()
{
    ...
    $count = $sql->query("
        ...
        FROM lzauser U
            JOIN lzarole R
              ON U.lzarole_id = R.id
            JOIN lzapermission P
              ON P.lzarole_id = R.id
        WHERE U.username = ?
...
$config['LicenseName'] = 'nghiemle.vn';
...
$baseHost = isset($_SERVER['HTTP_X_FORWARDED_HOST']) ? $_SERVER['HTTP_X_FORWARDED_HOST'] : $_SERVER['HTTP_HOST'];
$baseUrl = strtolower($_SERVER['REQUEST_SCHEME']) . '://' . $baseHost . ROOT_FOLDER . "root/app/client/res/";
...
ATTENTION: The trailing slash is required.
*/
$baseDir = resolveUrl(ROOT_FOLDER . "root/app/client/res/");
...
$config['ResourceType'][] = Array(
        ...
        'maxSize' => 0,
        'allowedExtensions' => '7z,aiff,asf,avi,bmp,csv,doc,docx,fla,flv,gif,gz,gzip,jpeg,jpg,mid,mov,mp3,mp4,mpc,mpeg,mpg,ods,odt,pdf,png,ppt,pptx,pxd,qt,ram,rar,rm,rmi,rmvb,rtf,sdc,sitd,swf,sxc,sxw,tar,tgz,tif,tiff,txt,vsd,wav,wma,wmv,xls,xlsx,zip,css,scss,map,js,eot,woff,woff2,ijmap,svg,ttf,otf',
        'deniedExtensions' => '');

$config['ResourceType'][] = Array(
        'name' => 'Compressed',
        'url' => $baseUrl . 'compressed',
        'directory' => $baseDir . 'compressed',
        'maxSize' => 0,
        'allowedExtensions' => 'zip,rar,7z,gz,gzip',
        'deniedExtensions' => '');
...
$config['ResourceType'][] = Array(
        ...
        'maxSize' => 0,
        'allowedExtensions' => 'bmp,gif,jpeg,jpg,png,psd,ai,ico',
        'deniedExtensions' => '');
...
$config['ResourceType'][] = Array(
        ...
        'maxSize' => 0,
        'allowedExtensions' => 'swf,mov,flv,mp4,avi,wmv,rm,ram,rmvb',
        'deniedExtensions' => '');
if (strpos($_SESSION['actual_link'], '/system') !== false)
{
    $config['ResourceType'][] = Array(
            'name' => 'Fonts',
            'url' => $baseUrl . 'fonts',
            'directory' => $baseDir . 'fonts',
            'maxSize' => 0,
            'allowedExtensions' => 'eot,woff,woff2,ijmap,svg,ttf,otf',
            'deniedExtensions' => '');

    $config['ResourceType'][] = Array(
            'name' => 'Layouts',
            'url' => $baseUrl . 'layouts',
            'directory' => $baseDir . 'layouts',
            'maxSize' => 0,
            'allowedExtensions' => 'html,tpl,php',
            'deniedExtensions' => '');

    $config['ResourceType'][] = Array(
            'name' => 'Styles',
            'url' => $baseUrl . 'styles',
            'directory' => $baseDir . 'styles',
            'maxSize' => 0,
            'allowedExtensions' => 'css,scss,map',
            'deniedExtensions' => '');

    $config['ResourceType'][] = Array(
            'name' => 'Scripts',
            'url' => $baseUrl . 'scripts',
            'directory' => $baseDir . 'scripts',
            'maxSize' => 0,
